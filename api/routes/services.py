...
def toggle_service(user_id):
    try:
        ...
        # Для ошибок бизнес-логики возвращаем 200 + success=false,
        # чтобы фронтенд мог показать message без network-level ошибки.
        if result.get('success'):
            return jsonify(result), 200
        return jsonify({'success': False, 'message': result.get('error', 'Failed to toggle service')}), 200
    except Exception as exc:
        logger.error(f'Error toggling user service {user_id}: {exc}')
...
def restart_user_service(user_id):
    try:
        ...
        if result.get('success'):
            return jsonify({'success': True, 'service_name': service_name, 'username': username, 'message': f'Service {service_name} restarted'})
        return jsonify({'success': False, 'message': result.get('error', 'Failed to restart service'), 'service_name': service_name, 'username': username}), 200
    except Exception as exc:
        logger.error(f'Error restarting user service {user_id}: {exc}')
        return jsonify({'success': False, 'message': str(exc)}), 500
